...
public class BossShiny implements BossPlaceholder {
    @Override
    public GenericResult handle(@Nullable Raid raid, @Nullable Boss boss, Boolean prioritizeRaid, List<String> args) {
        if (prioritizeRaid || boss == null) {
            if (raid == null) {
                raid = getRaid(args);
                if (raid == null) {
                    if (boss != null) return GenericResult.valid(boss.pokemonDetails.shiny);
                    return GenericResult.invalid("No Active Raid");
                }
            }
            return GenericResult.valid(raid.bossPokemon.getShiny());
        }

        return GenericResult.valid(boss.pokemonDetails.shiny);
    }

    @Override
    public List<String> id() {
        return List.of("boss_shiny");
    }
}
