...
public class BossDynamaxLevel implements BossPlaceholder {
    @Override
    public GenericResult handle(@Nullable Raid raid, @Nullable Boss boss, Boolean prioritizeRaid, List<String> args) {
        if (prioritizeRaid || boss == null) {
            if (raid == null) {
                raid = getRaid(args);
                if (raid == null) {
                    if (boss != null) return GenericResult.valid(boss.pokemonDetails.dynamaxLevel);
                    return GenericResult.invalid("No Active Raid");
                }
            }
            return GenericResult.valid(raid.bossPokemon.getDmaxLevel());
        }

        return GenericResult.valid(boss.pokemonDetails.dynamaxLevel);
    }

    @Override
    public List<String> id() {
        return List.of("boss_dmax_level", "boss_dynamax_level");
    }
}
