...
public class BossName implements BossPlaceholder {
    @Override
    public GenericResult handle(@Nullable Raid raid, @Nullable Boss boss, Boolean prioritizeRaid, List<String> args) {
        if (prioritizeRaid || boss == null) {
            if (raid == null) {
                ...
                if (raid == null) {
                    if (boss != null) {
                        return GenericResult.valid(boss.bossDetails.displayName);
                    }
                    return GenericResult.invalid("No Active Raid");
                }
            }
            return GenericResult.valid(raid.boss.bossDetails.displayName);
        }
