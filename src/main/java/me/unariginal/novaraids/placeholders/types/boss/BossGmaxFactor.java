...
public class BossGmaxFactor implements BossPlaceholder {
    @Override
    public GenericResult handle(@Nullable Raid raid, @Nullable Boss boss, Boolean prioritizeRaid, List<String> args) {
        if (prioritizeRaid || boss == null) {
            if (raid == null) {
                raid = getRaid(args);
                if (raid == null) {
                    if (boss != null) return GenericResult.valid(boss.pokemonDetails.gmaxFactor);
                    return GenericResult.invalid("No Active Raid");
                }
            }
            return GenericResult.valid(raid.bossPokemon.getGmaxFactor());
        }

        return GenericResult.valid(boss.pokemonDetails.gmaxFactor);
    }

    @Override
    public List<String> id() {
        return List.of("boss_gmax_factor");
    }
}
