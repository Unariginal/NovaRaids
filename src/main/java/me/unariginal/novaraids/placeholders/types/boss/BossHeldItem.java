...
public class BossHeldItem implements BossPlaceholder {
    @Override
    public GenericResult handle(@Nullable Raid raid, @Nullable Boss boss, Boolean prioritizeRaid, List<String> args) {
        if (prioritizeRaid || boss == null) {
            if (raid == null) {
                raid = getRaid(args);
                if (raid == null) {
                    if (boss != null) return GenericResult.valid(boss.pokemonDetails.heldItem.getName().getString());
                    return GenericResult.invalid("No Active Raid");
                }
            }
            return GenericResult.valid(raid.bossPokemon.heldItem().getName().getString());
        }

        return GenericResult.valid(boss.pokemonDetails.heldItem.getName().getString());
    }

    @Override
    public List<String> id() {
        return List.of("boss_held_item");
    }
}
