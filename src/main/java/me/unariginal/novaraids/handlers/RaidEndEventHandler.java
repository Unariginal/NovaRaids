...
public class RaidEndEventHandler {
    ...
    private static ActionResult raidEndPre(Raid raid) {
        Event event = Event.getEvent("raid_end_pre", raid.bossInfo.raidDetails.events.raidEnd.pre);
        if (event != null) RaidEventHandler.runEvent(event, raid, null,false);
        return ActionResult.PASS;
    }

    private static ActionResult raidEndPost(Raid raid) {
        Event event = Event.getEvent("raid_end_post", raid.bossInfo.raidDetails.events.raidEnd.post);
