...
public class BossDamagedEventHandler {
    ...
    private static ActionResult bossDamagedPre(Raid raid, int damage) {
        Event event = Event.getEvent("boss_damaged_pre", raid.bossInfo.raidDetails.events.bossDamaged.pre);
    ...
    private static ActionResult bossDamagedPost(Raid raid, int damage) {
        Event event = Event.getEvent("boss_damaged_post", raid.bossInfo.raidDetails.events.bossDamaged.post);
        if (event != null) RaidEventHandler.runEvent(event, raid, damage, false);
        return ActionResult.PASS;
    }
}
