...
public class RaidStartEventHandler {
    ...
    private static ActionResult raidStartPre(Raid raid) {
        Event event = Event.getEvent("raid_start_pre", raid.bossInfo.raidDetails.events.raidStart.pre);
        if (event != null) RaidEventHandler.runEvent(event, raid, null,true);
        return ActionResult.PASS;
    }

    private static ActionResult raidStartPost(Raid raid) {
        Event event = Event.getEvent("raid_start_post", raid.bossInfo.raidDetails.events.raidStart.post);
