...
**Please Review The Default Config Generation Gain Better Insight Into These Changes.**
...
**Config Changes:**
- Added `hide_other_catch_encounters` to the raid settings. This option will hide other player's catch encounters during the catch phase.
...
- Added `hide_other_pokemon_in_raid` to the raid settings. This option will hide other player's pokemon in the same raid as the player. If this is false, pokemon will be teleported away from the raid boss to remain out of players' way.
...
- Removed `raid_list_gui_title`. Now specified in it's gui config.
- Removed `raid_queue_gui_title`. Now specified in it's gui config.
- Removed `contraband_gui_title`. Now specified in it's gui config.
...
- Added `boss_id`, this is the ID that is referenced in other configs, commands, and with the `%boss%` placeholder. Previously this was just the file name.
...
- Moved `minimum_level` to Raid Details section.
- Moved `do_catch_phase` to Raid Details section.
...
- Added contraband section for boss specific contraband.
- Added bossbars section to reference a bossbar for each phase for this boss, this will override the selected boss bars in the category settings.
...
- This section allows for specifying details of raids within this category. Including:
  ...
  - The minimum and maximum players allows in a raid. 
...
- This section allows for distibuting reward pools to players that fall under certain placement specifications.
...
- Places allow for requiring the player to have dealt at least 1 damage to the boss.
...
**Config Info:**
- Specify the timezone that schedules will use here
- 3 types of schedules are usable: "random", "specific", and "cron".
- Each schedule allows for a category or boss to be selected at random based on weight specified. If a category is selected, a boss will be randomly selected from that category.
...
- Cron schedules allow for very customizable repetative starting of raids. Following the format: "`Seconds` `Minutes` `Hours` `Day Of Month` `Month` `Day Of Week`". Use this website to generate cron expressions: https://freeformatter.com/cron-expression-generator-quartz.html
